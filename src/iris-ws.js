...
var Ws = (function () {
    ...
    function Ws(endpoint, protocols) {
        ...
        this.conn.onopen = (function (evt) {
            _this.fireConnect(evt);
            _this.isReady = true;
            return null;
        });
        this.conn.onclose = (function (evt) {
            _this.fireDisconnect(evt);
            return null;
        });
    ...
    Ws.prototype.fireConnect = function (evt) {
        for (var i = 0; i < this.connectListeners.length; i++) {
            this.connectListeners[i](evt);
        }
    };
    Ws.prototype.OnDisconnect = function (fn) {
        this.disconnectListeners.push(fn);
    };
    Ws.prototype.fireDisconnect = function (evt) {
        for (var i = 0; i < this.disconnectListeners.length; i++) {
            this.disconnectListeners[i](evt);
        }
    };
